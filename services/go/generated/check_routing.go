...
func AddCheckRoutes(router *vestigo.Router, checkService check.Service) {
	...
	router.Get("/check/empty", func(res http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			...
			log.WithFields(logCheckEmpty).Error(errorResponse.Message)
	...
	router.Post("/check/empty_response", func(res http.ResponseWriter, req *http.Request) {
		...
		if !strings.Contains(contentType, "application/json") {
			...
			log.WithFields(logCheckEmptyResponse).Warn(errorResponse.Message)
			res.Header().Set("Content-Type", "application/json")
			res.WriteHeader(400)
			json.NewEncoder(res).Encode(errorResponse)
		...
		if err != nil {
			errorResponse := models.InternalServerError{fmt.Sprintf("Error returned from service implementation: %s", err.Error())}
			log.WithFields(logCheckEmptyResponse).Error(errorResponse.Message)
			res.Header().Set("Content-Type", "application/json")
			res.WriteHeader(500)
			json.NewEncoder(res).Encode(errorResponse)
	...
	router.Get("/check/forbidden", func(res http.ResponseWriter, req *http.Request) {
		...
		if err != nil {
			errorResponse := models.InternalServerError{fmt.Sprintf("Error returned from service implementation: %s", err.Error())}
			log.WithFields(logCheckForbidden).Error(errorResponse.Message)
			res.Header().Set("Content-Type", "application/json")
			res.WriteHeader(500)
			json.NewEncoder(res).Encode(errorResponse)
