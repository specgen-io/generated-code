...
public class EchoController {
	...
	public ResponseEntity<String> echoBodyArray(@RequestBody String bodyStr) {
		...
		try {
			requestBody = objectMapper.readValue(bodyStr, new TypeReference<List<String>>() {});
		} catch (IOException e) {
			logger.error("Failed to deserialize request body {}", e.getMessage());
			logger.info("Completed request with status code: {}", HttpStatus.BAD_REQUEST);
			return new ResponseEntity<>(HttpStatus.BAD_REQUEST);
